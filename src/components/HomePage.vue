<template>
  <div class="container">
    <div
      class="logo"
      :class="{
        'shift-left-logo': hoveredButton === 'search',
        'shift-right-logo': hoveredButton === 'join'
      }"
    >
    ...
    <p class="intro">
      Welcome to the PortAL Linking Artists and Scientists: a database designed to weave art into the fabric of the scientific ecosystem.
    </p>
    <div class="title">
      <div class="title-buttons">
        <div
          class="button-group search-group"
          :class="{ 'shift-left': hoveredButton === 'join' }"
          @mouseenter="hoveredButton = 'search'"
          @mouseleave="hoveredButton = null"
        >
          <p class="description">Are you a scientist looking for an artist?</p>
          <CustomButton to="/SearchArtists" text="Search" />
        </div>
        <div
          class="button-group join-group"
          :class="{ 'shift-left': hoveredButton === 'join' }"
          @mouseenter="hoveredButton = 'join'"
          @mouseleave="hoveredButton = null"
        >
          <p class="description">Are you an artist interested in scientific work?</p>
          <CustomButton @click="openSignUpModal" text="Join" class="small-button" />
        </div>
      </div>
    </div>
  </div>
...
import { openSignUpModal } from '@/composables/useAuthModal';
...
.logo svg path {
  stroke-width: .3px;
  animation: dashArray 11s ease-in-out infinite, dashOffset 10s linear infinite;
}
